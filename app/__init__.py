...
def process_message():
    ...
    if (not case):
        ...
        if (user_input_test == '1'):
            ...
            session['mistakes'] = 0
            mistakes = session.get('mistakes', 0)
            cursor.execute(config.add_sighting_sql)
            ...
            session['mistakes'] = 0
            mistakes = session.get('mistakes', 0)
            cursor.execute(config.add_evidence_sql)
            session['row_id'] = cursor.lastrowid
            connection.commit()
    ...
    if (case == 1):
        ...
        if (counter == 1):
            ...
            if (user_input_test.upper() == 'RESTART'):
                cursor.execute(config.update_sighting_restart_sql, (1, session['row_id'],))
                connection.commit()
                session.clear()
                session['counter'] = 1
                message.body(prompts.welcome)
                ...
                cursor.execute(config.update_sighting_address_sql, (address, lat, lon, session['row_id']))
                connection.commit()
                ...
                mistakes = session['mistakes']
                if (mistakes == 3):
                    cursor.execute(config.update_sighting_address_sql, (user_input, 0, 0, session['row_id']))
                    cursor.execute(config.update_sighting_mistake_sql, (1, session['row_id']))
                    connection.commit()
                    session.clear()
                    message.body(prompts.mistakes_prompt)
                    response.append(message)
            ...
            if (user_input_test.upper() == 'RESTART'):
                ...
                cursor.execute(config.update_sighting_in_out_sql, (0, session['row_id']))
                connection.commit()
                message.body(prompts.dead_or_alive)
                session['counter'] = counter + 1
                session['mistakes'] = 0
            elif (user_input_test == '2'):
                cursor.execute(config.update_sighting_in_out_sql, (1, session['row_id']))
                connection.commit()
                message.body(prompts.dead_or_alive)
                session['counter'] = counter + 1
                session['mistakes'] = 0
            ...
            if (user_input_test.upper() == 'RESTART'):
                ...
                cursor.execute(config.update_sighting_dead_alive_sql, (0, session['row_id']))
                cursor.execute(config.update_sighting_finished_sql, (1, session['row_id']))
                connection.commit()
                message.body(prompts.sighting_picture)
                session['counter'] = counter + 1
                session['mistakes'] = 0
            elif (user_input_test == '2'):
                cursor.execute(config.update_sighting_dead_alive_sql, (1, session['row_id']))
                cursor.execute(config.update_sighting_finished_sql, (1, session['row_id']))
                connection.commit()
                message.body(prompts.sighting_picture)
                session['counter'] = counter + 1
            ...
            if (user_input_test.upper() == 'RESTART'):
                ...
                cursor.execute(config.update_sighting_image_sql, (filepath, session['row_id']))
                connection.commit()
                session.clear()
        ...
        if (counter == 1):
            ...
            lat, lon, address = geocode(user_input.replace('\n', ' '))
            if (user_input_test.upper() == 'RESTART'):
                cursor.execute(config.update_evidence_restart_sql, (1, session['row_id']))
                connection.commit()
                session.clear()
                session['counter'] = 1
                message.body(prompts.welcome)
                ...
                cursor.execute(config.update_evidence_address_sql, (address, lat, lon, session['row_id']))
                connection.commit()
                ...
                mistakes = session['mistakes']
                if (mistakes == 3):
                    cursor.execute(config.update_evidence_address_sql, (user_input, 0, 0, session['row_id']))
                    cursor.execute(config.update_evidence_mistake_sql, (1, session['row_id']))
                    connection.commit()
                    session.clear()
                    message.body(prompts.mistakes_prompt)
                    response.append(message)
            ...
            if (user_input_test.upper() == 'RESTART'):
                ...
                cursor.execute(config.update_evidence_category_sql, (1, 0, session['row_id']))
                cursor.execute(config.update_evidence_finished_sql, (1, session['row_id']))
                connection.commit()
                session['counter'] = counter + 1
                session['mistakes'] = 0
                message.body(prompts.evidence_picture)
            elif (user_input_test == '2'):
                cursor.execute(config.update_evidence_category_sql, (0, 1, session['row_id']))
                cursor.execute(config.update_evidence_finished_sql, (1, session['row_id']))
                connection.commit()
                session['counter'] = counter + 1
                session['mistakes'] = 0
            ...
            if (user_input_test.upper() == 'RESTART'):
                ...
                with open(filepath, 'wb') as f:
                    image_url = request.values['MediaUrl0']
                    f.write(requests.get(image_url).content)
                cursor.execute(config.update_evidence_image_sql, (filepath, session['row_id']))
                connection.commit()
                session.clear()
                message.body(prompts.report_complete)
            elif (user_input_test.upper() == 'DONE'):
